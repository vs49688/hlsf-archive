...
int CHudMessage::VidInit( void )
{
	...
	m_HUD_title_mission = gHUD.GetSpriteIndex( "title_mission" );
	m_HUD_title_failed = gHUD.GetSpriteIndex( "title_failed" );
...
void CHudMessage::Reset( void )
{
	...
	memset(&mMissionFailed, 0, sizeof(client_textmessage_t));
}
...
{
	if(iMissionFailed)
		return DrawMissionFailed(flTime);
	else
		return DrawGameTitle(flTime);

}

int CHudMessage::DrawMissionFailed(float fTime)
{
	int i, drawn;
	...
	if ( m_gameMissionFailedTime > 0 )
	{
		float localTime = gHUD.m_flTime - m_gameMissionFailedTime;
		float brightness;

		// Maybe timer isn't set yet
		if ( m_gameMissionFailedTime > gHUD.m_flTime )
			m_gameMissionFailedTime = gHUD.m_flTime;

		if ( localTime > (mMissionFailed.fadein + mMissionFailed.holdtime + mMissionFailed.fadeout) )
			m_gameMissionFailedTime = 0;
		else
		{
			brightness = FadeBlend( mMissionFailed.fadein, mMissionFailed.fadeout, mMissionFailed.holdtime, localTime );

			int halfWidth = gHUD.GetSpriteRect(m_HUD_title_mission).right - gHUD.GetSpriteRect(m_HUD_title_mission).left;
			int fullWidth = halfWidth + gHUD.GetSpriteRect(m_HUD_title_failed).right - gHUD.GetSpriteRect(m_HUD_title_failed).left;
			int fullHeight = gHUD.GetSpriteRect(m_HUD_title_mission).bottom - gHUD.GetSpriteRect(m_HUD_title_mission).top;

			int x = XPosition( mMissionFailed.x, fullWidth, fullWidth );
			int y = YPosition( mMissionFailed.y, fullHeight );


			SPR_Set( gHUD.GetSprite(m_HUD_title_mission), brightness * mMissionFailed.r1, brightness * mMissionFailed.g1, brightness * mMissionFailed.b1 );
			SPR_DrawAdditive( 0, x, y, &gHUD.GetSpriteRect(m_HUD_title_mission) );

			SPR_Set( gHUD.GetSprite(m_HUD_title_failed), brightness * mMissionFailed.r1, brightness * mMissionFailed.g1, brightness * mMissionFailed.b1 );
			SPR_DrawAdditive( 0, x + halfWidth, y, &gHUD.GetSpriteRect(m_HUD_title_failed) );
...
int CHudMessage::DrawGameTitle( float fTime )
{
	int i, drawn;
	client_textmessage_t *pMessage;
	float endTime;
...
void CHudMessage::ShowMissionFailed()
{
	memset(&mMissionFailed, 0, sizeof(client_textmessage_t));

	mMissionFailed.x = -1;
	mMissionFailed.y = -1;
	mMissionFailed.effect = 0;
	mMissionFailed.r1 = 180;
	mMissionFailed.g1 = 180;
	mMissionFailed.b1 = 180;
	mMissionFailed.fadein = 5.0;
	mMissionFailed.fadeout = 4.0;
	mMissionFailed.holdtime = 99999;
