...
//  hud_msg.cpp
//
#include "VSSound.h" //AJH - Killar MP3
#include "hud.h"
#include "cl_util.h"
#include "parsemsg.h"
#include "r_efx.h"
...
int CHud :: MsgFunc_VSys( const char *pszName, int iSize, void *pbuf ) //vs49688
{
	BEGIN_READ( pbuf, iSize );

	gMP3.ParseString( READ_STRING() );

	return 1;
}

int CHud :: MsgFunc_SetInput(const char *pszName, int iSize, void *pbuf )
{
	extern void SwitchInput(int iInput);
	//extern int g_iInputType;
	BEGIN_READ( pbuf, iSize );
	SwitchInput(READ_LONG());
	//g_iInputType = READ_LONG();
