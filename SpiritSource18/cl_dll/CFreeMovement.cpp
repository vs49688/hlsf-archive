...
void CFreeMovement::CL_AdjustAngles(float frametime, float *viewangles)
{
	float	speed;
	float	up, down;
...
void CFreeMovement::CL_CreateMove(float frametime, struct usercmd_s *cmd, int active)
{	
	float spd;
	vec3_t viewangles;
...
void CFreeMovement::IN_MouseMove (float frametime, usercmd_t *cmd, float * deltaAngles)
{
	int		mx, my;
	//vec3_t viewangles;
	...
	if ( !iMouseInUse && !g_iVisibleMouse && !gHUD.m_iIntermission )
	{
		...
		if ( (in_strafe.state & 1) || (lookstrafe->value && (in_mlook.state & 1) ))
			...
			deltaAngles[YAW] -= m_yaw->value * mouse_x;

		if ( (in_mlook.state & 1) && !(in_strafe.state & 1))
		{
			deltaAngles[PITCH] += m_pitch->value * mouse_y;
		}
		else
		{
	...
	//(this->*SetViewAngles)( (float *)viewangles );
...
void CFreeMovement::IN_JoyMove ( float frametime, usercmd_t *cmd, float * deltaAngles )
{
	float	speed, aspeed;
		...
		case AxisForward:
			if ((joy_advanced->value == 0.0) && (in_jlook.state & 1))
			{
				...
				{		
					...
					{
						deltaAngles[PITCH] -= (fAxisValue * joy_pitchsensitivity->value) * aspeed * cl_pitchspeed->value;
					}
					else
					{
						deltaAngles[PITCH] += (fAxisValue * joy_pitchsensitivity->value) * aspeed * cl_pitchspeed->value;
		...
		case AxisTurn:
			...
			else
			{
				...
				if (fabs(fAxisValue) > joy_yawthreshold->value)
				{
					...
					{
						deltaAngles[YAW] += (fAxisValue * joy_yawsensitivity->value) * aspeed * cl_yawspeed->value;
					}
					else
					{
						deltaAngles[YAW] += (fAxisValue * joy_yawsensitivity->value) * speed * 180.0;
					}
		...
		case AxisLook:
			if (in_jlook.state & 1)
			{
				if (fabs(fAxisValue) > joy_pitchthreshold->value)
				{
					...
					{
						deltaAngles[PITCH] += (fAxisValue * joy_pitchsensitivity->value) * speed * 180.0;
					}
					V_StopPitchDrift();
				}
				else
...
void CFreeMovement::IN_Move (float frametime, usercmd_t *cmd, float *deltaAngles)
{
