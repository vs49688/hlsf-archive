...
{
	extern void CMD_ThirdPerson();
	extern void HUD_DisplayMissionFailed();
	extern int g_iInputType;
	extern int g_iPrevInput;
	extern int g_iKeepInput;
	//void CMD_HidePlayer(void);
	...
	//pLocalPlayer->pev->model = NULL;

	if(g_iKeepInput)
		g_iPrevInput = g_iInputType;
	
	g_iInputType = INPUT_NORMAL;

	HUD_DisplayMissionFailed();
	gMP3.DeathPlay();
	gMP3.StopAll();
}
...
{
	extern void CMD_FirstPerson(void);
	extern int g_iInputType;
	extern int g_iPrevInput;
	extern int g_iKeepInput;

	if(g_iKeepInput)
		g_iInputType = g_iPrevInput;
	else
		g_iInputType = 0;
...
//void __declspec(dllexport) VSys_ClientDeath(void *pPlayer);
