...
int __MsgFunc_VSys(const char *pszName, int iSize, void *pbuf )
{
	return gHUD.MsgFunc_VSys( pszName, iSize, pbuf );
}

int __MsgFunc_SetInput(const char *pszName, int iSize, void *pbuf)
{
	return gHUD.MsgFunc_SetInput(pszName, iSize, pbuf);
}
...
void __CmdFunc_VSysParseCommand(void)
{
	__asm{int 3}
}
...
//	HOOK_MESSAGE( KeyedELight ); //LRC
	...
	if(gMP3.Initialize())
	{
		...
		HOOK_MESSAGE( VSys );	//VSys
		//HOOK_COMMAND( "vsys", VSysParseCommand );
	}
...
void HUD_DisplayMissionFailed()
{
	gHUD.m_Message.ShowMissionFailed();
}