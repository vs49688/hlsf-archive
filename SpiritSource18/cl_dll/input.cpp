...
// This is initialized in InitInput()
CBaseMovement *g_pInput[MAX_INPUTS];
...
void SwitchInput(int iInput)
{

	g_iInputType = iInput;

	if((iInput > MAX_INPUTS) || (iInput < 0))
	{
		g_iInputType = 0;
	}
	else if(iInput == 0)
	{
		...
		fAngles.y = 0;
		fAngles.z = 0;
		gEngfuncs.SetViewAngles((float*)fAngles);
...
void DLLEXPORT CL_CreateMove ( float frametime, struct usercmd_s *cmd, int active )
{	
	g_pInput[g_iInputType]->CL_CreateMove(frametime, cmd, active);
}
...
void InitInput (void)
{
	...
	g_pInput[INPUT_NORMAL] = new CBaseMovement;
	g_pInput[INPUT_FREE] = new CFreeMovement;
}
