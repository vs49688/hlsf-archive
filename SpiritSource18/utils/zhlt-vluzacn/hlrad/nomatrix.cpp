    ...
    if (patch2)
    {
        ...
        if (DotProduct(patch->origin, plane2->normal) > (PatchPlaneDist(patch2) + MINIMUM_PATCH_DISTANCE))
        {
            ...
            if (
                ...
                && (TestLine(patch->origin, patch2->origin) == CONTENTS_EMPTY)
#else
...
{	
    ...
    if (emitpatch)
    {
        ...
        if (DotProduct(backorigin, emitplane->normal) > (PatchPlaneDist(emitpatch) + MINIMUM_PATCH_DISTANCE))
        {
            ...
            if (
                (DotProduct(emitpatch->origin, backnormal) > (DotProduct(backorigin, backnormal) + MINIMUM_PATCH_DISTANCE))
#ifdef HLRAD_WATERBLOCKLIGHT
                && (TestLine(backorigin, emitpatch->origin) == CONTENTS_EMPTY)
#else
                && (TestLine_r(0, backorigin, emitpatch->origin) == CONTENTS_EMPTY)
#endif
                && (!TestSegmentAgainstOpaqueList(backorigin, emitpatch->origin
#ifdef HLRAD_HULLU
					, transparency
#endif
#ifdef HLRAD_OPAQUE_STYLE
