...
int TestPointOpaque_r (int nodenum, const vec3_t point)
{
	opaquenode_t *thisnode;
	vec_t dist;
	if (nodenum < 0)
	{
		if (g_dleafs[-nodenum-1].contents == CONTENTS_SOLID)
			return 1;
		else
			return 0;
	}
	...
	if (dist > ON_EPSILON * 1.5)
	{
		return TestPointOpaque_r (thisnode->children[0], point);
	}
	if (dist < -ON_EPSILON * 1.5)
	{
		return TestPointOpaque_r (thisnode->children[1], point);
	}
	{
		int facenum;
		for (facenum = thisnode->firstface; facenum < thisnode->firstface + thisnode->numfaces; facenum++)
	...
	return TestPointOpaque_r (thisnode->children[0], point)
		|| TestPointOpaque_r (thisnode->children[1], point);
}

int TestPointOpaque (int modelnum, const vec3_t modelorigin, const vec3_t point)
{
	opaquemodel_t *thismodel = &opaquemodels[modelnum];
	vec3_t newpoint;
	...
	return TestPointOpaque_r (thismodel->headnode, newpoint);
}
#endif
