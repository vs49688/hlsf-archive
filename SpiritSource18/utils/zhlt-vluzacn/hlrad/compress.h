	...
	(vector_type t, void *s, const float *f1, const float *f2, const float *f3)
{
	...
	case VECTOR48:
		...
		if (float_istoobig (*p2))
			m[0] |= bitput (bitget (~0, 0, 16), 16, 32);
	...
	case VECTOR24:
		...
		else
		{
			...
			i1 = float_istoobig (*p1)? ~0 : (bitget (*p1, 0, 23) | bitput (1, 23, 24)) >> (1 + max - bitget (*p1, 23, 31));
			i2 = float_istoobig (*p2)? ~0 : (bitget (*p2, 0, 23) | bitput (1, 23, 24)) >> (1 + max - bitget (*p2, 23, 31));
			i3 = float_istoobig (*p3)? ~0 : (bitget (*p3, 0, 23) | bitput (1, 23, 24)) >> (1 + max - bitget (*p3, 23, 31));
		}
		if (t == VECTOR32)
			m[0] = 0
