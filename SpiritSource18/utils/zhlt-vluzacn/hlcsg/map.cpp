...
brush_t *CopyBrush (entity_t *entity, const brush_t *brush)
{
	if (entity->firstbrush + entity->numbrushes != g_nummapbrushes)
	{
		Error ("CopyBrush: internal error.");
	}
...
static void     CopySKYtoCLIP(const brush_t* const b)
{
	...
	newbrush->detaillevel = 0;
	newbrush->chopdown = 0;
	newbrush->chopup = 0;
#ifdef ZHLT_CLIPNODEDETAILLEVEL
	newbrush->clipnodedetaillevel = 0;
#endif
#endif
...
#endif
{
    ...
    while (ok)
    {
...
#ifdef HLCSG_CUSTOMHULL
		{
			...
			else if (!strncasecmp (g_token, "CLIP", 4))
			{
#ifdef HLCSG_PASSBULLETSBRUSH
				strcpy (g_token, "SKIP");
#else
    ...
    if (contents == CONTENTS_ORIGIN)
    {
		...
		if (*ValueForKey (mapent, "origin"))
			Error ("Only one ORIGIN brush allowed.");
#endif
	...
	if (*ValueForKey (&g_entities[b->entitynum], "zhlt_usemodel"))
	{
		...
		if (b->contents == CONTENTS_ORIGIN)
		{
			...
			{
				side = &g_brushsides[b->firstside + j];
				autowad_PushName(side->td.name);
			}
		}
#endif
		g_numbrushsides -= b->numsides;
		memset (&g_brushsides[g_numbrushsides], 0, b->numsides * sizeof (side_t));
		g_nummapbrushes--;
		memset (&g_mapbrushes[g_nummapbrushes], 0, sizeof (brush_t));
		mapent->numbrushes--;
		return;
	}
#endif
    ...
    if (contents == CONTENTS_BOUNDINGBOX)
    {
		if (*ValueForKey (mapent, "zhlt_minsmaxs"))
			Error ("Only one BoundingBox brush allowed.");
        char            string[MAXTOKEN];
        vec3_t          mins, maxs;
		char			*origin = NULL;
		if (*ValueForKey (mapent, "origin"))
	...
	{
		brush_t *newb = CopyBrush (mapent, b);
...
bool            ParseMapEntity()
{
	...
	if (!g_noutf8)
	{
		epair_t *ep;
		char *newvalue;
		if (!strcmp (ValueForKey(mapent, "classname"), "game_text"))
			for (ep = mapent->epairs; ep; ep = ep->next)
				if (!strcmp(ep->key, "message"))
				{
					newvalue = ANSItoUTF8 (ep->value);
					Free (ep->value);
					ep->value = newvalue;
				}
	}
