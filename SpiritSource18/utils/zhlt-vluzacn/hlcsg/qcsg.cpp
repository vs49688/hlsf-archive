...
void     ReuseModel ()
{
	...
	for (i = 1; i < g_numentities; i++)
	{
		name = ValueForKey (&g_entities[i], "zhlt_usemodel");
		if (name[0])
		{
			...
			if (j == g_numentities)
			{
				if (!stricmp (name, "null"))
					...
					Error ("Reuse Model: ( Entity %d ) can not find entity '%s'.\n", i, name);
			}
			else if (j > i)
				// to prevent precache error in case of .mdl/.spr and wrong result of EntityForModel in case of map model
				Error ("Reuse Model: '%s' ( Entity %d ) must be created prior to the entity that use its model ( Entity %d ). \n"
				"You can cut and paste the latter to solve this problem.", name, j, i);
			else
			{
				SetKeyValue (&g_entities[i], "model", ValueForKey (&g_entities[j], "model"));
			}
			//DeleteKey (&g_entities[i], "zhlt_usemodel");
		}
	}
}
#endif
...
#ifdef HLCSG_WADCFG_NEW
	Log("    -wadcfgfile file : Reads in wad configuration\n");
#endif
...
int             main(const int argc, char** argv)
{
	...
	if (g_hullfile && !q_exists (g_hullfile))
	{
		char temp[_MAX_PATH];
		char test[_MAX_PATH];
	...
	if (g_nullfile && !q_exists (g_nullfile))
	{
		char temp[_MAX_PATH];
		char test[_MAX_PATH];
		safe_strncpy (temp, g_Mapname, _MAX_PATH);
	...
	if (g_wadcfgfile)
	{
		...
		{
			Error("Could not find wad configuration file '%s'\n", g_wadcfgfile);
		}
