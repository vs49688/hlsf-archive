...
static surfchain_t* ReadSurfs(FILE* file)
{
    ...
    double          v[3];
    int             line = 0;
#ifdef HLCSG_HLBSP_DOUBLEPLANE //debug
    ...
    while (1)
    {
        ...
        {
#ifdef HLCSG_HLBSP_DOUBLEPLANE //debug
			Developer (DEVELOPER_LEVEL_FLUFF, "inaccuracy: average %.8f max %.8f\n", inaccuracy_total / inaccuracy_count, inaccuracy_max);
#endif
            break;
        }
#ifdef ZHLT_DETAILBRUSH
...
#ifdef HLBSP_REMOVEHULL2
	Log("    -nohull2       : Don't generate the hull 2 (hull for large monster)\n");
#endif
...
int             main(const int argc, char** argv)
{
	...
	if (g_subdivide_size % 16 != 0)
	{
		Warning ("Subdivide size must be a multiple of 16");
		g_subdivide_size = 16 * (g_subdivide_size / 16);
	}
#endif
