...
void            SubdivideFace(face_t* f, face_t** prevptr)
{
    ...
    for (axis = 0; axis < 2; axis++)
    {
        while (1)
        {
			...
			const int maxlightmapsize = g_subdivide_size / 16 + 1;
			...
			lightmapmins = (int)floor (mins / 16.0 - 0.05); // the worst case
			lightmapmaxs = (int)ceil (maxs / 16.0 + 0.05); // the worst case
			...
			plane.dist = ((splitpos + 0.5) * 16.0 - tex->vecs[axis][3]) / v;
#else
