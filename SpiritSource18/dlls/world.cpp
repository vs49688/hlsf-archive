...
int CGlobalState::Save( CSave &save )
{
	...
	if ( !save.WriteFields( "cGLOBAL", "GLOBAL", this, m_SaveData, HLARRAYSIZE(m_SaveData) ) )
		return 0;
	
	pEntity = m_pList;
	for ( i = 0; i < m_listCount && pEntity; i++ )
	{
		if ( !save.WriteFields( "cGENT", "GENT", pEntity, gGlobalEntitySaveData, HLARRAYSIZE(gGlobalEntitySaveData) ) )
			return 0;
...
//
	...
	for (i = 0; i < HLARRAYSIZE(gDecals); i++ )
