...
void ClientCommand( edict_t *pEntity )
{
	...
	else if( FStrEq(pcmd, "vsys") )
	{
		if(CMD_ARGC() > 1)
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgVSys, NULL, ENT(pev) );
				WRITE_STRING( g_engfuncs.pfnCmd_Args() );
			MESSAGE_END();
		}
	}
...
// player pain sounds
	...
	// HLSF-Specific Stuff
	UTIL_PrecacheOther("env_deadplayer");
