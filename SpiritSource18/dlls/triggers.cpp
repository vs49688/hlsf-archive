...
void PlayCDTrack( int iTrack, int iSong)
{
	...
	if ( iTrack == -1 )
	{
		CLIENT_COMMAND ( pClient, "vsys stopcda" );
	...
	else
	{
		...
		if(iSong) sprintf( string, "playaudio %s\n", STRING(iSong) ); // playaudio is already hooked
		else sprintf(string, "vsys playcda %3d 0\n", iTrack);
...
/*
class CDeathRotCam : public CTriggerCamera
{
public:
	void Spawn( void );
	void KeyValue( KeyValueData *pkvd );
...
LINK_ENTITY_TO_CLASS( trigger_deathrotcam, CDeathRotCam );
