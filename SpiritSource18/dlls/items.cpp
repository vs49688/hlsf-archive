...
{	
	...
	UTIL_PrecacheOther( "item_supersuit" );
...
void CWorldItem::Spawn( void )
{
	...
	case 45: // ITEM_SUIT:
		...
		break;
	case 46: // ITEM_SUPERSUIT:
		pEntity = CBaseEntity::Create( "item_supersuit", pev->origin, pev->angles );
	}
...
class CItemSuit : public CItem
{
	...
	BOOL MyTouch( CBasePlayer *pPlayer )
	{
		...
		pPlayer->m_iHideHUD |= ( HIDEHUD_ARMOR | HIDEHUD_FLASHLIGHT );
...
class CItemSuperSuit : public CItem
{
	void Spawn( void )
	{ 
		Precache( );
...
LINK_ENTITY_TO_CLASS(item_supersuit, CItemSuperSuit);
...
class CItemBattery : public CItem
{
	...
	BOOL MyTouch( CBasePlayer *pPlayer )
	{
		...
		if ((pPlayer->pev->armorvalue < MAX_NORMAL_BATTERY) && (pPlayer->m_iHasSuperSuit)/*(pPlayer->pev->weapons & (1<<WEAPON_SUIT))*/)
		{
			int pct;
			char szcharge[64];
