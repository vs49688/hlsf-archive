...
#define IMPLEMENT_SAVERESTORE(derivedClass,baseClass) \
	...
	{\
		if ( !baseClass::Save(save) )\
			return 0;\
		if (pev->targetname)\
			return save.WriteFields( STRING(pev->targetname), #derivedClass, this, m_SaveData, HLARRAYSIZE(m_SaveData) );\
		else\
			return save.WriteFields( STRING(pev->classname), #derivedClass, this, m_SaveData, HLARRAYSIZE(m_SaveData) );\
	}\
