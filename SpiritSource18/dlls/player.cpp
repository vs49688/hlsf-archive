...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] =
{
	...
	DEFINE_FIELD( CBasePlayer, m_iHasSuperSuit, FIELD_INTEGER ),	// Do we have the supersuit
	DEFINE_FIELD( CBasePlayer, m_iHasGravSuit, FIELD_INTEGER ),
...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	if(pDeadPlayer)
	{
		pDeadPlayer->pev->origin = pev->origin;
		pDeadPlayer->pev->origin.z -= 36; // 36 is how high the player model is off the ground

		pDeadPlayer->pev->angles.x = 0; // We have to be flat

		//DON'T update the angles, otherwise the dead body will move with the camera
		//pDeadPlayer->pev->angles = pev->angles;
	}
...
//	CheckDesiredList( ); //LRC
	...
	if ( g_pGameRules && g_pGameRules->FAllowFlashlight(this) && m_iHasSuperSuit)
...
int CBasePlayer::Save( CSave &save )
{
	...
	return save.WriteFields( "cPLAYER", "PLAYER", this, m_playerSaveData, HLARRAYSIZE(m_playerSaveData) );
}
...
{
	if ( !g_pGameRules->FAllowFlashlight(this) )
	{
		return;
	}
...
void CBasePlayer::StopDeath()
{
	...
	UTIL_ScreenFade( this, Vector(0, 0, 0), 1, NULL, 0, DEATH_FADE_FLAGS );
