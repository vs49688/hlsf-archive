...
void COtis :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case OTIS_AE_DRAW:
		...
		pev->body = m_iBaseBody + OTIS_BODY_GUNDRAWN;
		m_fGunDrawn = TRUE;
		break;

	case OTIS_AE_HOLSTER:
		// change bodygroup to replace gun in holster
		pev->body = m_iBaseBody + OTIS_BODY_GUNHOLSTERED;
		m_fGunDrawn = FALSE;
		break;
...
void COtis :: Spawn()
{
	...
	pev->body			= m_iBaseBody + OTIS_BODY_GUNHOLSTERED; // gun in holster
	m_fGunDrawn			= FALSE;
...
{
	if ( pev->body < m_iBaseBody + OTIS_BODY_GUNGONE && !(pev->spawnflags & SF_MONSTER_NO_WPN_DROP))
	{// drop the gun!
		Vector vecGunPos;
		Vector vecGunAngles;

		pev->body = m_iBaseBody + OTIS_BODY_GUNGONE;
