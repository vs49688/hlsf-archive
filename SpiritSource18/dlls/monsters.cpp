...
int CBaseMonster::Save( CSave &save )
{
	if ( !CBaseToggle::Save(save) )
		return 0;
	if ( pev->targetname )
		return save.WriteFields( STRING(pev->targetname), "CBaseMonster", this, m_SaveData, HLARRAYSIZE(m_SaveData) );
	else
		return save.WriteFields( STRING(pev->classname), "CBaseMonster", this, m_SaveData, HLARRAYSIZE(m_SaveData) );
}
